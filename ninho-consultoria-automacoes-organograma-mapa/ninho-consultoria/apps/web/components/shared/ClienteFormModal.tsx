...
export default function ClienteFormModal({
  ...
  const [buscaCoordenadasMsg, setBuscaCoordenadasMsg] = useState<string | null>(null);
  ...
  async function buscarCoordenadas() {
    const endereco = [form.logradouro, form.cidade, form.uf, "Brasil"].filter(Boolean).join(", ");
    if (!form.cidade.trim()) {
      setBuscaCoordenadasMsg("Preencha ao menos a cidade antes de buscar.");
      return;
    }
    setBuscandoCoordenadas(true);
    setBuscaCoordenadasMsg(null);
    try {
      ...
      if (resultados?.[0]) {
        setForm((f) => ({ ...f, latitude: resultados[0].lat, longitude: resultados[0].lon }));
        setBuscaCoordenadasMsg(null);
      } else {
        setBuscaCoordenadasMsg("Endereço não encontrado — tente deixar só cidade e UF, ou preencha manualmente.");
      }
    } catch {
      setBuscaCoordenadasMsg("Não foi possível buscar agora. Preencha manualmente ou tente de novo.");
    }
          ...
          <Secao titulo="Endereço">
            ...
            <div className="flex items-end gap-3 mt-4">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-lg text-[12.5px] font-semibold border border-[#e4e6ea] text-[#3f434d] hover:border-primary hover:text-primary transition-colors disabled:opacity-60 shrink-0 h-[41px]"
              >
                {buscandoCoordenadas ? <Loader2 size={13} className="animate-spin" /> : <MapPin size={13} />}
                Buscar pelo endereço
              </button>
            </div>
            {buscaCoordenadasMsg && <p className="text-[11px] text-[#e08a00] mt-1.5">{buscaCoordenadasMsg}</p>}
            <p className="text-[11px] text-[#9aa0ac] mt-1.5">
              Preenche o mapa na Visão Geral automaticamente. Se a busca não achar o local exato, ajuste manualmente.
            </p>
          </Secao>
